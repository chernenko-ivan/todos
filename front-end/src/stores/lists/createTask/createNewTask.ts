import {ListsState, List, NewTaskPayload, TaskPrepare, Task} from "../@types";
...
export const createTask: CaseReducer<ListsState, PayloadAction<NewTaskPayload>> = (state, action) => ({
    ...state,
...
export const addNewTaskToLists = (lists: List[], payload: NewTaskPayload): List[] => compose(
    insertTaskIntoLists(lists),
    createNewTask
)(payload);


// insertIntoLists insert task into list that had id equals to task.id.
export const insertTaskIntoLists = _.curry((lists: List[], task: Task): List[] => lists.map(
    list => list.id === task.listId
        ? insertIntoList(list, task)
        : list
));

// insertIntoList insert task into list and return new list obj.
export const insertIntoList = _.curry((list: List, task: Task): List => ({
    ...list,
    tasks: [
        ...list.tasks,
        task as Task
...
export const createNewTask = (data: NewTaskPayload): TaskPrepare => <TaskPrepare>compose(
    setId,
    setIsDoneFalse,
    setCreatedDate
)(data);
...
export const setId = (data: TaskPrepare): TaskPrepare => ({...data, id: generateTaskId()});
