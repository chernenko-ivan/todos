...
// editTask
export const editTask: CaseReducer<ListsState, PayloadAction<Task>> = (
	state,
	action,
) => ({
	...state,
...
// editTaskInLists
export const editTaskInLists = (lists: List[], task: Task): List[] =>
	lists.map(list =>
		list.id === task.listId ? editTaskInList(list, task) : list,
	);
